...
public class Alice5WalkingControllerParameters extends WalkingControllerParameters
{
   ...
   private Alice5SteppingParameters steppingParameters;
   private final Alice5ICPControllerParameters icpControllerParameters;
   private final Alice5StepAdjustmentParameters stepAdjustmentParameters;
   private JointPrivilegedConfigurationParameters jointPrivilegedConfigurationParameters = new Alice5JointPrivilegedConfigurationParameters();
   ...
   private final Alice5MomentumOptimizationSettings momentumOptimizationSettings;

   // USE THESE FOR Real Alice5 Robot and sims when controlling pelvis height instead of CoM.
   ...
   public Alice5WalkingControllerParameters(Alice5VersionInterface version, RobotTarget target, Alice5JointMap jointMap, Alice5PhysicalProperties physicalProperties)
   {
      this(version, target, jointMap, physicalProperties, new Alice5ContactPointParameters(jointMap, physicalProperties));
   }

   public Alice5WalkingControllerParameters(Alice5VersionInterface version,
                                                   RobotTarget target,
                                                   Alice5JointMap jointMap,
                                                   Alice5PhysicalProperties physicalProperties,
                                                   RobotContactPointParameters<RobotSide> contactPointParameters)
   {
      ...
      toeOffParameters = new Alice5ToeOffParameters(physicalProperties);
      momentumOptimizationSettings = new Alice5MomentumOptimizationSettings(target, jointMap, 2 + contactPointParameters.getAdditionalContactNames().size());
      swingTrajectoryParameters = new Alice5SwingTrajectoryParameters();
      steppingParameters = new Alice5SteppingParameters(physicalProperties);
      icpControllerParameters = new Alice5ICPControllerParameters();
      stepAdjustmentParameters = new Alice5StepAdjustmentParameters();
   ...
   public Map<String, Pose3D> getOrCreateBodyHomeConfiguration()
   {
      ...
      for (RobotSide robotSide : RobotSide.values)
      { // Hand taskspace home pose can be computed from jointspace home pose with Alice5TaskspaceHomePoseCalculator
   ...
   public void setSteppingParameters(Alice5SteppingParameters steppingParameters)
   {
...
//      return new Alice5NaturalPostureParameters(jointMap);
