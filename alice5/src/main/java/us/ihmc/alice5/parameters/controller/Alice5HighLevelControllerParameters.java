...
public class Alice5HighLevelControllerParameters implements HighLevelControllerParameters
{
   protected final Alice5VersionInterface alice5Version;
   protected final Alice5JointMap jointMap;
   protected final RobotTarget target;
   private final Alice5StandPrepSetPoints standPrepSetPoints;

   public Alice5HighLevelControllerParameters(Alice5VersionInterface alice5Version, Alice5JointMap jointMap, RobotTarget target)
   {
      this.alice5Version = alice5Version;
      this.jointMap = jointMap;
      this.target = target;
      standPrepSetPoints = new Alice5StandPrepSetPoints(jointMap);
   }
   ...
   protected List<GroupParameter<JointDesiredBehaviorReadOnly>> getDesiredJointBehaviorForWalkingNotLoaded()
   {
      ...
      if (target == RobotTarget.REAL_ROBOT)
      {
         ...
         configureBehavior(behaviors, jointMap, SpineJointName.SPINE_ROLL, EFFORT, 100.0, 6.0, maxPosError, maxVelError, velScale);
   ...
   public List<GroupParameter<JointDesiredBehaviorReadOnly>> getDesiredJointBehaviorForHangingAround()
   {
      ...
      if (target == RobotTarget.REAL_ROBOT)
      {
         ...
         configureBehavior(behaviors, jointMap, SpineJointName.SPINE_ROLL, EFFORT, 1000.0, 3.0, maxPosError, maxVelError, velScale);
      ...
      else
      {
         ...
         configureBehavior(behaviors, jointMap, SpineJointName.SPINE_ROLL, POSITION, 1500.0, 150.0, maxPosError, maxVelError, velScale);
   ...
   public List<GroupParameter<JointDesiredBehaviorReadOnly>> getDesiredJointBehaviorForCalibration()
   {
      ...
      if (target == RobotTarget.REAL_ROBOT)
      {
         ...
         configureBehavior(behaviors, jointMap, SpineJointName.SPINE_ROLL, EFFORT, 500.0, 0.0, maxPosError, maxVelError, velScale);
